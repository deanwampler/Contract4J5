...
Dean Wampler 
http://www.Contract4J5.org
http://www.aspectprogramming.com/Contract4J5
Contract4J5 is a project of Aspect Research Associates
(http://www.aspectresearchassociates.com/)

***** NOTE *****
The most up-to-date and accurate version of this readme is on the contract4j.org website: http://contract4j.org/contract4j/c4j5!
...
Impatient? If you want to see what Java code looks like with Contract4J5 in 
...
Annotation Form:
	Contract4J5 v1.X.Y  --> Contract4J5 v.0.1.X.Y  
The "5" emphasizes that it requires Java 5 annotations

"ContractBeans" Form:
	Contract4J5 v2.X.Y --> Contract4JBeans v.0.2.X.Y
...
Contract4J5 supports "Design by Contract"(TM) programming in Java. a programming
practice introduced by Bertrand Meyer and incorporated in the Eiffel 
programming language in the 1980's. 

The Contract4J5 project is sponsored by Aspect Research Associates 
...
The "@Contract" annotation tells Contract4J5 that "MyClass" defines tests. The
...
If a test fails, Contract4J5 throws an unchecked exception, ContractError, after
...
(required by Jexl) to use Contract4J5. If you build Contract4J5 yourself, you 
will also need JUnit. If you build the Spring Framework example, which shows
how to configure Contract4J5 using Spring, you will need v1.2.5 or later of 
Spring.

You can use the installed "contract4j5.jar" file as is. If you want to rebuild
Contract4J5, use the ant driver script "build.sh" or "build.bat", edit the
corresponding "env.sh" or "env.bat" file and change the environment variable
...
To use Contract4J5 in your projects, begin by adding the jar to your class
path. If you are not already using the AspectJ "ajc" compiler to compile your 
Java and AspectJ sources, then add a binary weaving step to your build or 
modify the way you invoke test runs to use load-time weaving.
...
"compile.test" target uses javac to compile the JUnit test code. The 
"project-test.jar" target uses ajc to do binary weaving, where the compiled 
class files are read by ajc, aspects are woven into them (note the
"aspectpath" option to ajc) and the contract4j5-test.jar file is output. The 
ant/targets.xml defines the "binaryWeaveTemplate" used for this process.

To see what the ajc command does, invoke ant with the option 
...
LICENSE.txt - The Apache 2 license file for Contract4J5
...
which is not part of the normal runtime deployment.

In the separate "Contract4J5WithSpring" directory, you will find the following:
...
Building "all" creates the jar files "Contract4J5.jar" and "Contract4J5-test.jar"
...
An example of using Contract4J5 with Load-Time Weaving (LTW) is included. 
WARNING: it requires AspectJ 1.5.2a to work, due to a bug in 1.5.2 that 
prevents Contract4J5 from being used with load-time weaving.
...
3) Load-Time Weaving
This is the least disruptive approach for using Contract4J5 in a pure Java 
...
missing "classes" are actually aspects:
  ...
  UsageEnforcement
  Contract4J5ConfigurationEnsurer
...
This was discussed in depth above, in the section titled "How Does Contract4J5 
...
if an annotation is empty, that is it doesn't define a test expression, then
...
When a test fails due to a test error, a subclass of ContractError is thrown,
...
currently Contract4J5 does not distinguish between test "bugs" and actual
test failures.
...
Here are some API examples. Note that the internal restructuring done in the v0.6.0 release changed the wiring API significantly compared to previous releases. 
...
3) The type "Contract4J5" was an aspect in pre-v0.6.0 releases. It is now a class
and a singleton instance is used. Hence,
   Contract4J5.setEnabled(Contract4J5.TestType.Pre, true);
is now
   Contract4J5.getInstance().setEnabled(Contract4J5.TestType.Pre, true);
...
2) Java 5 (JAVA_HOME)
3) AspectJ 1.5 (ASPECTJ_HOME) The 1.5.2a or later release is recommended (required if you do load-time weaving).
3) Ant 1.6.2+ (ANT_HOME)
4) Jexl 1.0 (Jexl_HOME)
5) Commons Logging (COMMONS_LOGGING_HOME)
6) Spring (SPRING_HOME) - optional

Also define "Contract4J5_HOME" to be the ".../contract4j5_060/contract4j5"
...
"contract4j5.jar". For binary or load-time weaving, see the examples in
the Contract4J build files. 
...
2) Implement more exhaustive tests. While Contract4J5 comes with a significant
...
- Added an example using the Spring Framework v1.2.5 to configure Contract4J5
...
compatable, if not easier with new 2.0 features. However, using Contract4J5 
with Spring v2.0 was not tested.

- Greatly improved the options for using properties files to "wire" Contract4J5, 
...
Contract4J5's non-test code and to create the Contract4J5.jar file.
...
used, the one set in the "Contract4J5" class.
...
- "Contract4J5" is no longer an aspect, but a class. The aspect code was moved
...
previously set on "Contract4J5" are not changed to use "aspectOf", as just 
described for the aspects. Instead, use for example,
  Contract4J5.getInstance().setEnabled(Contract4J5.TestType.Pre, true);

Bugs:
	1) Seems to require spring to be present to build! This is a bug in the ant scripts.
...
Contract4J5 v0.1. The workaround is to put all parameter tests in a method 
precondition test.
...
http://www.Contract4J5.org/ is the home page for Contract4J5 and Contract4JBeans.
...
There you will find more information and whitepapers on Contract4J5 and 
...
Contract4J5, http://www-128.ibm.com/developerworks/java/library/j-aopwork17.html.
It introduces "Design by Contract" and how Contract4J5 supports it in Java. The 
...
a talk in the Industry Track on Contract4J5, specifically on the lessoned about
...
Contract4J5 in the ACP4IS workshop. See http://www.aosd.net/2006/ for more
...
plans to incorporate Contract4J5 into the new standard library for AspectJ5 that 
...
partially inspired Contract4J5. http://barter.sourceforge.net/. 
